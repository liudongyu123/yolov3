...
class LoadImagesAndLabels(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            # Albumentations 平移、翻转
            img, labels = self.albumentations(img, labels)
            nl = len(labels)  # update after albumentations

            # HSV color-space 色彩扰动
...
def load_image(self, i):
    ...
    if im is None:  # not cached in ram
        ...
        if npy and npy.exists():  # load npy
            ...
            # 根据index索引从路径列表中获取当前图片的路径
            path = self.img_files[i]
            # 根据路径读取图像，cv会读取成BGR格式
            im = cv2.imread(path)  # BGR
            assert im is not None, f'Image Not Found {path}'
        h0, w0 = im.shape[:2]  # orig hw
        # self.img_size 设置的是预处理后输出的图片尺寸
...
def load_mosaic(self, index):
    ...
    labels4, segments4 = [], []
    s = self.img_size  # S=640 图片的大小
    # 拼接点的位置 yc,xc
    yc, xc = (int(random.uniform(-x, 2 * s + x)) for x in self.mosaic_border)  # mosaic center x, y
    # 从dataset中随机寻找三张图像进行拼接，再随机选三张图片的index
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # top left
            ...
            # 计算马赛克图像中的坐标信息(将图像填充到马赛克图像中)   w=640  h = 480  
            # 马赛克图像：(x1a,y1a)左上角 (x2a,y2a)右下角
        ...
        labels, segments = self.labels[index].copy(), self.segments[index].copy() # 深拷贝，防止修改原数据
        if labels.size:
    ...
    labels4 = np.concatenate(labels4, 0)
     # labels4[:, 1:]中的所有元素的值（位置信息）必须在[0, 2*s]之间,小于0就令其等于0,大于2*s就等于2*s   out: 返回
    for x in (labels4[:, 1:], *segments4):
        np.clip(x, 0, 2 * s, out=x)  # use with clip 防止越界
