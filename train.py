    ...
    if RANK in [-1, 0]:
        ...
        if not resume:
            ...
            if not opt.noautoanchor:
                # 现在的anchor是通过聚类得到的，
                # 如果更换数据集就要检查数据集的groundtrues是否和anchor差不多，
                # 差得多要重新聚类生成anchor
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni <= nw:
                ...
                for j, x in enumerate(optimizer.param_groups):
                    ...
                    # [hyp['warmup_bias_lr'] if j == 2 else 0.0, x['initial_lr'] * lf(epoch)] == [0,0.01]
                    # 当横轴处于[0,17745],纵轴处于[0,0.01],学习率是一条直线时，进行插值
